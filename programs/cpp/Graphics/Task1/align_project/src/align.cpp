            ...
            bool isInterp, bool isSubpixel, double subScale)
{
    ...
    if (isSubpixel)
        newImage = resize(newImage, 1.0/subScale);
    if (isPostprocessing)
    {
...
Image grad(Image sobelX, Image sobelY)
{
    int i,j,rows = sobelX.n_rows, cols = sobelY.n_cols;
    Image new_image(rows,cols);
    for (i = 0; i < rows; i++)
        for (j = 0; j < cols; j++)
        {
            uint r,g,b,Gr,Gg,Gb;
            tie(Gr,Gg,Gb) = sobelX(i,j);
            tie(r,g,b) = sobelY(i,j);
            Gr = sqrt(Gr*Gr + r*r);
            Gg = sqrt(Gg*Gg + g*g);
            Gb = sqrt(Gb*Gb + b*b);
            new_image(i,j) = make_tuple(Gr,Gg,Gb);
        }
    return new_image;
}
...
Image resize(Image src_image, double scale) {
    ...
    for (i = 0; i < rows; i++)
        ...
        {
            uint r,g,b;
            tie(r,g,b) = src_image(i/scale, j/scale);
...
Image canny(Image src_image, int threshold1, int threshold2) {
    return grad(sobel_x(src_image),sobel_y(src_image));
}
